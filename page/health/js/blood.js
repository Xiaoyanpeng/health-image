;(function () {
	...
	ShowChart.prototype = {
		...
		event:function () {
			var that = this;
			var $typeSrc = $('#typeSrc li'),
				$bloodTypeSrc = $('#bloodTypeSrc li'),
				$timeTypeSrc = $('#timeTypeSrc li');

			$typeSrc.on('click',function () {
				var isActive = $(this).hasClass('active');
				var type = $typeSrc.index($(this)) + 1;
				...
				function glocoseFn() {
					...
					$('.blood-glocose').eq(0).addClass('active')
				}
				function pressFn() {
					that.type = 2;
					that.bloodType = 8;
					...
					$('.blood-glocose').hide();
					$('.blood-press').show();
					$('.blood-press').eq(0).addClass('active')
				}
				...
				glucoseChart.setOption(option = {
					graphic: [
						...
						},
						{
							...
							bounding: 'raw',
							style: {
								image: 'img/shengchenghuaxiang_btn.png',
								width: 260,
								height: 80
							}
						}
